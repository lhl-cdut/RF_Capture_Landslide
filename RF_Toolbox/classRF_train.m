...
function model = classRF_train(p_train, t_train, ntree, mtry, extra_options)
  ...
  % if ~exist('strata', 'var');      strata   = 1;     end
    if ~exist('sampsize', 'var')   
        if (replace) 
            ...
            sampsize = ceil(0.632 * size(p_train, 1));%向上取整
        end
    end
  ...
  % if ~exist('proximity','var');   proximity = 1;      end  % will handle these two late
  ...
  % if ~exist('corr_bias', 'var');   corr_bias = FALSE;  end
    ...
    if ~exist('ntree', 'var') || ntree <= 0  %优化一次以后换这个
		ntree = 500;
        DEFAULTS_ON = 1;
    end

    if ~exist('mtry','var') || mtry <= 0 || mtry > size(p_train, 2) %优化一次以后换这个
        mtry = floor(sqrt(size(p_train, 2)));
    end
    
    addclass = isempty(t_train);
    
    if (~addclass && length(unique(t_train)) < 2)
        error('need atleast two classes for classification');
    end
    [N, D] = size(p_train);
    
    if N == 0
        error(' data (X) has 0 rows'); 
    end
    ...
    if ~isempty(find(isnan(p_train), 1));  error('NaNs in X');   end
    if ~isempty(find(isnan(t_train), 1));  error('NaNs in Y');   end
    ...
    nclass = length(unique(t_train));
    
    if ~exist('cutoff', 'var') 
        cutoff = ones(1, nclass) * (1 / nclass);
    else
        if sum(cutoff) > 1 || sum(cutoff) < 0 || ~isempty(find(cutoff <= 0, 1)) || length(cutoff) ~= nclass
...
%      nsample = n;
    ...
    if Stratify
        ...
        if ( ~isempty(find(sampsize <= 0, 1)) || nsum == 0)
    ...
    [nrnodes, ntree, xbestsplit, classwt, cutoff, treemap, nodestatus, nodeclass, ...
        bestvar, ndbigtree, mtry, outcl, counttr, prox, impmat, impout, impSD, errtr, inbag] ...
        = mexClassRF_train(p_train',int32(Y_new),length(unique(t_train)),ntree,mtry,int32(ncat), ... 
    ...
    model.nclass       = length(unique(t_train));
    model.outcl        = outcl;
